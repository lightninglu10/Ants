...
"""
Person A: Patrick Lu
Person B: Barber Waters

Write your answers to written problems here.

1.1) Simulate in class AntColony

1.2) The method strategy in class Hive makes the bees exit the hive.
First, an exit is made by checking if it is first an entrance. Then, when the
assault plan is run, the bees will move to the exit made. The time the bees come
is determined by the assault plans.

1.3) In the mixed_layout function, the places are registered to the colony,
which effectively creates the places. By changing the length and the tunnels in
mixed_layout, that changes the number of places. By changing the moat frequency,
the amount of water changes.

1.4) The armor attribute is how much damage it can take. When the armor reaches 0, it is taken out of the game.
"""
################
# Core Classes #
################
...
class Ant(Insect):
    ...
    def can_contain(self, ant):
        if self.container:
            if (ant.container==False and self.ant==None):
                return True
            else:
                return False
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        def nearest(place, min_range, max_range):
            while True:
                if type(place) != Place:
                    return None
                elif place == hive:
                    ...
                    return nearest(place.entrance,min_range-1,max_range-1)
                elif random_or_none(place.bees)!= None:
                    return random_or_none(place.bees)
                elif place.entrance == None:
                    return None
                else:
                    return nearest(place.entrance,min_range-1,max_range-1)
        return nearest(self.place, self.min_range, self.max_range)
...
def test_layout(queen, register_place, length=10, tunnels=1):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        if (self.armor-amount)<=0:
            for every in self.place.bees[:]:
                every.reduce_armor(3)
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        while bee.armor>0:
            bee.reduce_armor(1)
        "Less efficient than remove bee but passes tests"
